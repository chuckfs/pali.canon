...
class TestCitationExtraction:
    ...
    def test_extract_with_periods(self):
        ...
        # After fix, we normalize to "SN 35.28" and "MN 21"
        assert "SN 35.28" in cits
        assert "MN 21" in cits

    def test_extract_with_colons(self):
        """Test citations with colons."""
        # After fix, we normalize "SN:35:28" -> "SN 35.28"
    ...
    def test_extract_anguttara(self):
        ...
        assert "AN 4.159" in cits
        assert "AN 3.65" in cits
    ...
    def test_extract_khuddaka_texts(self):
        ...
        NOTE:
        - Book names alone ("Dhammapada verse") are not citations.
        - We require a reference number (e.g., Dhp 1, Ud 1.10, It 112).
        """
        cits = _extract_citations("Dhammapada 21 and Udāna 1.10 and Itivuttaka 112")
        assert "Dhp 21" in cits
        assert "Ud 1.10" in cits
        assert "It 112" in cits
...
class TestRegexPatterns:
    ...
    def test_nikaya_ref_pattern(self):
        ...
        # Should handle spacing / punctuation variations
        assert NIKAYA_REF_RE.search("SN35.28")
        assert NIKAYA_REF_RE.search("SN-35.28")
        assert NIKAYA_REF_RE.search("SN.35.28")
        assert NIKAYA_REF_RE.search("SN:35:28")

        # This input is ambiguous (space-separated segments); we do not require it
        # because it tends to create false positives in real text.
        # If you *want* to support it, we can broaden the regex safely later.
    ...
    def test_sutta_name_pattern(self):
        ...
        # Also allow sutta name without "Sutta"
        assert SUTTA_NAME_RE.search("Satipaṭṭhāna")
        assert SUTTA_NAME_RE.search("Ādittapariyāya")
